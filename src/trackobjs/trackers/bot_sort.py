...
from .utils.image_process import save_one_box
...
class BOTrack(STrack):
    ...
    Attributes:
        ...
        smooth_feat (numpy.ndarray): Smoothed feature vector.
        curr_feat (numpy.ndarray): Current feature vector.
        features (deque): A deque to store feature vectors with a maximum length defined by `feat_history`.
        alpha (float): Smoothing factor for the exponential moving average of features.
        mean (numpy.ndarray): The mean state of the Kalman filter.
        covariance (numpy.ndarray): The covariance matrix of the Kalman filter.
    ...
    Examples:
        ...
        >>> bo_track = BOTrack(tlwh=[100, 50, 80, 40], score=0.9, cls=1, feat=numpy.random.rand(128))
        >>> bo_track.predict()
        >>> new_track = BOTrack(tlwh=[110, 60, 80, 40], score=0.85, cls=1, feat=numpy.random.rand(128))
    ...
    def __init__(
        self, xywh: numpy.ndarray, score: float, cls: int, feat: Optional[numpy.ndarray] = None, feat_history: int = 50
    ):
        """
        Initialize a BOTrack object with temporal parameters, such as feature history, alpha, and current features.

        Args:
            xywh (numpy.ndarray): Bounding box coordinates in xywh format (center x, center y, width, height).
            score (float): Confidence score of the detection.
            cls (int): Class ID of the detected object.
            feat (numpy.ndarray, optional): Feature vector associated with the detection.
            feat_history (int): Maximum length of the feature history deque.

        Examples:
            Initialize a BOTrack object with bounding box, score, class ID, and feature vector
            >>> xywh = numpy.array([100, 150, 60, 50])
            >>> score = 0.9
            >>> cls = 1
            >>> feat = numpy.random.rand(128)
    ...
    def update_features(self, feat: numpy.ndarray) -> None:
        """Update the feature vector and apply exponential moving average smoothing."""
        feat /= numpy.linalg.norm(feat)
        self.curr_feat = feat
        ...
        self.smooth_feat /= numpy.linalg.norm(self.smooth_feat)
    ...
    def convert_coords(self, tlwh: numpy.ndarray) -> numpy.ndarray:
        """Convert tlwh bounding box coordinates to xywh format."""
        return self.tlwh_to_xywh(tlwh)

    @staticmethod
    def tlwh_to_xywh(tlwh: numpy.ndarray) -> numpy.ndarray:
...
class BOTSORT(BYTETracker):
    ...
    def init_track(self, results, img: Optional[numpy.ndarray] = None) -> List[BOTrack]:
        """Initialize object tracks using detection bounding boxes, scores, class labels, and optional ReID features."""
        if len(results) == 0:
            return []
        bboxes = results.xywhr if hasattr(results, "xywhr") else results.xywh
        bboxes = numpy.concatenate([bboxes, numpy.arange(len(bboxes)).reshape(-1, 1)], axis=-1)
    ...
    def get_dists(self, tracks: List[BOTrack], detections: List[BOTrack]) -> numpy.ndarray:
        ...
        if self.args.with_reid and self.encoder is not None:
            ...
            dists = numpy.minimum(dists, emb_dists)
        return dists
...
#     def __call__(self, img: numpy.ndarray, dets: numpy.ndarray) -> List[numpy.ndarray]:
