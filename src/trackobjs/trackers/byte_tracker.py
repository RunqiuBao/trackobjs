...
class STrack(BaseTrack):
    ...
    Attributes:
        ...
        _tlwh (numpy.ndarray): Private attribute to store top-left corner coordinates and width and height of bounding box.
        kalman_filter (KalmanFilterXYAH): Instance of Kalman filter used for this particular object track.
        mean (numpy.ndarray): Mean state estimate vector.
        covariance (numpy.ndarray): Covariance of state estimate.
    ...
    def __init__(self, xywh: List[float], score: float, cls: Any):
        ...
        self._tlwh = numpy.asarray(xywh2ltwh(xywh[:4]), dtype=numpy.float32)
    ...
    @staticmethod
    def multi_gmc(stracks: List["STrack"], H: numpy.ndarray = numpy.eye(2, 3)):
        ...
        if len(stracks) > 0:
            ...
            R = H[:2, :2]
            R8x8 = numpy.kron(numpy.eye(4, dtype=float), R)
            t = H[:2, 2]
    ...
    @staticmethod
    def tlwh_to_xyah(tlwh: numpy.ndarray) -> numpy.ndarray:
    ...
    @property
    def xywh(self) -> numpy.ndarray:
        """Get the current position of the bounding box in (center x, center y, width, height) format."""
        ret = numpy.asarray(self.tlwh).copy()
        ret[:2] += ret[2:] / 2
        return ret

    @property
    def xywha(self) -> numpy.ndarray:
        ...
        if self.angle is None:
            ...
            return self.xywh
        return numpy.concatenate([self.xywh, self.angle[None]])
...
class BYTETracker:
    ...
    def update(self, results, img: Optional[numpy.ndarray] = None, feats: Optional[numpy.ndarray] = None) -> numpy.ndarray:
        ...
        return numpy.asarray([x.result for x in self.tracked_stracks if x.is_activated], dtype=numpy.float32)
    ...
    def init_track(self, results, img: Optional[numpy.ndarray] = None) -> List[STrack]:
    ...
    def get_dists(self, tracks: List[STrack], detections: List[STrack]) -> numpy.ndarray:
